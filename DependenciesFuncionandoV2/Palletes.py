...
def pick_series_color_by_type(color_map, palettes, type_label_colors, special_label_colors,
    ...
    if norm in special_label_colors:
        color = special_label_colors[norm]
    else:
            ...
            if t == "misc":
                ...
                if idx < 10:
                    if base:
                        ...
                        color = _deterministic_hex(f"misc:{norm}:{idx}")
                else:
                    color = _deterministic_hex(f"misc:{norm}:{idx}")
            else:
                ...
                if pal_id is not None:
                    ...
                    if idx < len(pal):
                        ...
                        color = _deterministic_hex(f"{t}:{norm}:{idx}")
                else:
                    color = _deterministic_hex(f"unknown:{t}:{norm}:{idx}")

    # --- SAFETY NORMALIZATION ---
    color = color.strip().upper().lstrip("#")
