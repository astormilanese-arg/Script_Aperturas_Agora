...
def refresh_all_graphs(pptx_path: str, save: bool = True, with_window: bool = False) -> dict:
    """
    Re-renderiza gráficos/enlaces usando sólo PowerPoint:
      • Gráficos nativos: Chart.Refresh()
      • Objetos vinculados: LinkFormat.Update()
    Devuelve un dict con estadísticas.
    """
    def iter_shapes(shapes):
        # Recorre shapes, incluyendo grupos
        for i in range(1, shapes.Count + 1):
            shp = shapes.Item(i)
            if getattr(shp, "Type", None) == 6 and hasattr(shp, "GroupItems"):  # msoGroup
                gi = shp.GroupItems
    ...
    pres = pp.Presentations.Open(pptx_path, WithWindow=with_window)

    try:
        pp.DisplayAlerts = 1  # ppAlertsNone en muchas instalaciones
    except Exception:
        pass

    stats = {"charts_refreshed": 0, "links_updated": 0, "errors": 0}

    try:
        for sld in pres.Slides:
            # 1) Actualizar vínculos (imágenes/objetos enlazados)
            for shp in iter_shapes(sld.Shapes):
                try:
            ...
            # 2) Refrescar gráficos nativos
            for shp in iter_shapes(sld.Shapes):
                try:
                    if getattr(shp, "HasChart", False):
                        shp.Chart.Refresh()
                        stats["charts_refreshed"] += 1
                except Exception:
                    stats["errors"] += 1

        # Fallback de nivel presentación
        try:
            pres.UpdateLinks()
        except Exception:
            pass
...
def safe_dispatch(app_name):
    """Robust COM dispatch with multiple fallback strategies."""
    try:
        # First try normal dispatch
        return gencache.EnsureDispatch(app_name)
    except (SyntaxError, AttributeError, pywintypes.com_error) as e:
        print(f"COM cache error: {e}, attempting recovery...")
        ...
        try:
            ...
            if os.path.exists(cache_path):
                ...
                print(f"Cleared COM cache: {cache_path}")
            
            # Try again with cached dispatch
            return gencache.EnsureDispatch(app_name)
        except Exception:
            # Final fallback: dynamic dispatch (no cache)
            print("Using dynamic dispatch as fallback")
